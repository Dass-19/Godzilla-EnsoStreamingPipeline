...
def logs_productores():
    ...
    try:
        ...
        raise _sin_datos("Aún no hay logs de producers en HDFS") from None
    except HdfsError as error:
        raise _hdfs_caido(error) from error

    return _respuesta_exitosa(nombres, fuente=ruta_hdfs)


@app.get(
    "/api/logs",
    response_model=RespuestaAPI[list[dict[str, str]]],
    tags=["observabilidad"],
...
def logs_productor(
    ...
    try:
        ...
        raise _sin_datos(f"No hay logs para el producer {producer}") from None
    except HdfsError as error:
        raise _hdfs_caido(error) from error
    ...
    if not registros:
        raise _sin_datos(f"No hay logs para producer={producer} con esos filtros") from None

    return _respuesta_exitosa(registros, fuente=ruta_hdfs)
