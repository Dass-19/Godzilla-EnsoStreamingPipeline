...
def ingest_data():
    ...
    try:
        ...
        nino4 = ee.Geometry.Rectangle([160, -5, 180, 5]).union(
            ee.Geometry.Rectangle([-180, -5, -150, 5])
        )
        ...
        oisst_collection = ee.ImageCollection("NOAA/CDR/OISST/V2_1").filterDate(inicio, fin)
        ...
        for i in range(size):
            ...
            date_str = datetime.datetime.fromtimestamp(date_ms / 1000.0, datetime.UTC).strftime(
                "%Y-%m-%d"
            )
            ...
            stats_34 = (
                img.select(["sst", "anom"])
                .reduceRegion(
                    reducer=ee.Reducer.mean(), geometry=nino34, scale=25000, maxPixels=1e9
                )
                .getInfo()
            )
            ...
            stats_12 = (
                img.select(["sst", "anom"])
                .reduceRegion(
                    reducer=ee.Reducer.mean(), geometry=nino12, scale=25000, maxPixels=1e9
                )
                .getInfo()
            )
            ...
            stats_3 = (
                img.select(["sst", "anom"])
                .reduceRegion(reducer=ee.Reducer.mean(), geometry=nino3, scale=25000, maxPixels=1e9)
                .getInfo()
            )
            stats_4 = (
                img.select(["sst", "anom"])
                .reduceRegion(reducer=ee.Reducer.mean(), geometry=nino4, scale=25000, maxPixels=1e9)
                .getInfo()
            )
